...
## **Text to Add to `Understand_transformer.md`**

Since you're in **ask mode**, please add the following content after line 169 in `Understand_transformer.md`:

```markdown
## Training Strategies

### Training Objective: Per-Day Multi-Label Classification

The model is trained using a **per-day multi-label classification** approach, where it learns to predict which medical codes occur on each day given the patient's medical history and demographics.

#### Input Structure
- **Patient medical history**: Up to 200 days of temporal data
- **Demographics per day**: Age (in months) and gender code
- **Medical codes per day**: Up to 80 codes representing diagnoses, procedures, medications, and other medical events

#### Model Processing Flow

**1. Daily Code Aggregation (Level 1)**
- Daily encoder (`transformer_encoder_cd`) processes up to 80 codes within each day
- Uses self-attention to capture relationships between co-occurring codes
- Max pooling aggregates codes into a single 256-dimensional daily representation

**2. Temporal Pattern Learning (Level 2)**
- Temporal encoder (`transformer_encoder_dy`) processes the sequence of 200 daily representations
- Causal masking ensures the model only attends to past days (prevents future information leakage)
- 6 transformer layers with 16 attention heads learn disease progression and treatment patterns

**3. Output Predictions**
- Final decoder layer (`decoder_cd`) projects each day's representation to `target_cd_cnt` classes
- Produces probability distributions over possible medical codes for each day
- Output shape: `[batch_size, 200_days, target_cd_cnt]` with log-softmax activations

#### Loss Computation

**Negative Log-Likelihood Loss (NLLLoss)**
```python
# Training loop extracts predictions and targets
opt = model(x)  # [batch, 200, target_cd_cnt]
opt = opt.reshape(batch_size * len_dy, target_cd_cnt)  # Flatten to [batch*200, target_cd_cnt]

# Extract only actual days (exclude padding)
opt = torch.cat([opt[len_dy*i:len_dy*i+dt_cnt[i], :] for i in range(batch_size)], dim=0)

# Flatten target codes (each code occurrence is a separate label)
y = [item for sublist in y for item in sublist]  # List of target code indices
...
**Key Characteristics:**
- **Multi-label per day**: Multiple codes can occur on the same day, each treated as a separate classification target
- **Same-day reconstruction**: The model predicts which codes occur on day *t* given information up to and including day *t* (not forecasting day *t+1*)
- **Actual days only**: Loss is computed only on days with real data (using `dt_cnt` to mask out padded days)
- **Each code = one loss term**: If 5 codes occur on day 3, the model makes 5 predictions (all using day 3's representation) and accumulates 5 NLLLoss terms

#### Training Characteristics

**What the Model Learns:**
- Semantic relationships between medical codes (e.g., diabetes codes cluster together)
- Temporal patterns in disease progression and treatment sequences
- Context-dependent code occurrence (certain codes more likely given patient history and demographics)
- Co-occurrence patterns (codes that tend to appear together on the same day)

**What the Model Does NOT Predict:**
- ❌ Next day's codes (no temporal shift/forecasting)
- ❌ Sequential ordering of codes within a single day
- ❌ Exact timing within a day

**Causal Masking Behavior:**
- Prevents attention to future days (day *t* can only attend to days 0 through *t*)
- However, the model *does* see all codes on day *t* when predicting day *t*'s codes
- This makes it a **masked language modeling** style task rather than pure next-token prediction

#### Inference: Embedding Extraction

During inference (scoring), the model's purpose shifts from prediction to representation learning:

```python
# Extract embedding from the last actual day of data
intermedia_output = activation['transformer_encoder_dy']
patient_embedding = intermedia_output[dt_cnt[i], i, :]  # [256] dimensional vector
```

This 256-dimensional embedding captures:
- Complete patient medical history (up to 200 days)
- Disease progression patterns
- Treatment sequences
- Demographic context

These embeddings can then be used for downstream tasks such as:
- Risk stratification
