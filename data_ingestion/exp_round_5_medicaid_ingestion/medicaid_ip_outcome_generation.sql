...
WITH acute AS (
    ...
    FROM  
        `edp-prod-storage.edp_ent_sdoheir_cns.a964286_Medicaid_outcome_ip_cases_4_te_experiment`
    WHERE 
        ip_type = 'Acute'
    GROUP BY 
        individual_id
...
/*==============================================================================
  IP OUTCOME SUMMARY BY ENTITLEMENT CATEGORY
  
  Purpose: Analyze acute IP admission rates across Medicaid entitlement groups
  
  Requires: coa_population_category field in membership base
  
==============================================================================*/

WITH base_with_category AS (
    -- Join membership base with original eligibility data to get categories
    SELECT 
        m.individual_id
        , m.index_dt
        , e.coa_population_category
        -- Grouped categories (following 001a_Membership_Index.sh logic)
        , CASE 
            ...
            WHEN TRIM(e.coa_population_category) IN ('Dual Int DD', 'Dual Int NonLTSS')
                THEN 'Dual Int'
            WHEN TRIM(e.coa_population_category) IN ('CHIP')
                THEN 'CHIP'
            WHEN TRIM(e.coa_population_category) IN ('TANF')
                THEN 'TANF'
    ...
    FROM 
        `edp-prod-storage.edp_ent_sdoheir_cns.a834793_Medicaid` AS m
    INNER JOIN
        `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_ELIG_DATA_MBR_PER_MTH` AS e
            ON m.individual_id = e.asdb_member_key
            AND m.index_dt = CAST(e.asdb_elig_dt AS DATE)
)
, outcomes AS (
    SELECT 
        o.individual_id
        , o.acute_ip_flag
        , o.sum_acute_ip_admits
        , o.sum_acute_ip_cost
    FROM
        `edp-prod-storage.edp_ent_sdoheir_cns.a964286_Medicaid_outcome_ip_4_te_experiment` AS o
)
, joined AS (
    SELECT 
        b.coa_population_category
        , b.coa_population_group
        , o.acute_ip_flag
        , o.sum_acute_ip_admits
        , o.sum_acute_ip_cost
    FROM 
        base_with_category AS b
    LEFT JOIN
        outcomes AS o
            ON b.individual_id = o.individual_id
)
-- Summary by grouped category
SELECT 
    'GROUPED' AS summary_level
    , coa_population_group AS category
    , COUNT(*) AS total_members
    , SUM(acute_ip_flag) AS members_with_ip
    , ROUND(AVG(acute_ip_flag) * 100, 2) AS ip_rate_pct
    , SUM(sum_acute_ip_admits) AS total_ip_admits
    , ROUND(AVG(sum_acute_ip_admits), 2) AS avg_admits_per_member
FROM 
    joined
GROUP BY 
...
ORDER BY 
    summary_level, 
    CASE 
        WHEN category = 'All Members' THEN 0
        ELSE 1 
    END,
    ip_rate_pct DESC
;
