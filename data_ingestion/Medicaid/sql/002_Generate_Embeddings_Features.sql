...
CREATE OR REPLACE TABLE  `{{CM_MD_SCHEMA}}.{{PREFIX}}_MEMBER_SCORE_ENDING`
{{LABELS}}
AS
...
FROM 
    (SELECT DISTINCT asdb_member_key, index_dt FROM `{{CM_MD_SCHEMA}}.{{PREFIX}}_MEMBER_INDEX`) AS st
GROUP BY
      st.asdb_member_key
    , st.index_dt
;
...
CREATE OR REPLACE TABLE  `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1A_SCORE_ENDING`
{{LABELS}}
AS
...
(
    SELECT
        ...
        , clm.status_detail
    FROM 
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_MEMBER_SCORE_ENDING` AS base 
    LEFT JOIN
...
LEFT JOIN
    (SELECT src_specialty_cd, asdb_svc_prov_key FROM `{{CM_MD_SCHEMA}}.MD_TE_PROVIDER_DB_XWALK`) AS map
...
CREATE OR REPLACE TABLE  `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1B_SCORE_ENDING`
{{LABELS}}
AS
...
WITH base AS (
    SELECT 
        ...
        , indx.index_dt
    FROM
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1A_SCORE_ENDING` AS base
   LEFT JOIN
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_MEMBER_SCORE_ENDING` AS indx
...
CREATE OR REPLACE TABLE  `{{CM_MD_SCHEMA}}.{{PREFIX}}_O1_SCORE_ENDING`
{{LABELS}}
AS
...
WITH root0 AS (
    ...
    FROM 
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1A_SCORE_ENDING`
)
, root1 AS (
    SELECT 
        *
...
, x0 as (
    ...
    FROM 
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1A_SCORE_ENDING` AS base 
    LEFT JOIN 
        `{{CM_MD_SCHEMA}}.MD_TE_V9_W2IND` AS w2ind --replace with production table if possible
    ...
    UNION DISTINCT
        SELECT 
            ...
            , CASE WHEN w2ind.ind IS NULL THEN 0 
                ELSE w2ind.ind END AS ind
        FROM 
            `{{CM_MD_SCHEMA}}.{{PREFIX}}_D1B_SCORE_ENDING` AS base 
        LEFT JOIN
            `{{CM_MD_SCHEMA}}.MD_TE_V9_W2IND` AS w2ind --replace with production table if possible
...
CREATE OR REPLACE TABLE  `{{CM_MD_SCHEMA}}.{{PREFIX}}_O3_SCORE_ENDING`
{{LABELS}}
AS        
...
WITH x1 AS (
    ...
    FROM 
        `{{CM_MD_SCHEMA}}.{{PREFIX}}_O1_SCORE_ENDING`
)
, x2 AS (
    SELECT 
        * 
