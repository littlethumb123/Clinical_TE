...
create or replace table `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_archive_clm_ln_bk1`
...
select distinct
    ...
    inner join
        ( select
          ...
          from `edp-prod-hcbstorage.edp_hcb_core_cnsv.CLM_LN_X_ICD9_DX_ARC`   -- need to change at year end when changing archive year
          ...
          select
              ...
              ARRAY[STRUCT(split(trim(icd9_dx_cd),'.')[safe_offset(0)] as l,
                    substr(split(trim(icd9_dx_cd),'.')[safe_offset(1)],1,2) as r)] AS icd_array
          from `edp-prod-hcbstorage.edp_hcb_core_cnsv.CLM_LN_X_ICD9_DX_Y2021`   -- need to change at year end when changing archive year
          where cast(sequence_id as int) < 4 and icd9_dx_cd is not NULL and trim(icd9_dx_cd)!=""
        ) b
        on base.member_id = b.member_id and base.claim_line_id=b.claim_line_id
    LEFT JOIN UNNEST(b.icd_array) as var_st
;