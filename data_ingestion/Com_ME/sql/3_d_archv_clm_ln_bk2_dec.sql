-- changed name
-- Y2020 needs to be changed by turn of year

create or replace table `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_archive_clm_ln_bk2`
...
select distinct
    ...
    inner join
        ( select
          ...
          from `edp-prod-hcbstorage.edp_hcb_core_cnsv.CLM_LN_X_ICD9_DX_Y2020`    -- need to change at year end when changing archive year
