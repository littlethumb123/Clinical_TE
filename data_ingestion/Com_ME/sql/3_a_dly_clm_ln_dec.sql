-- chnaged `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_d1a_score_ending_tmp` to 
-- removed with block, moved condition to 1_a

create or replace table `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_daily_clm_ln`
...
as
    ...
    from `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_daily_claims` base
    inner join
        ( select
              member_id,
              claim_line_id,
