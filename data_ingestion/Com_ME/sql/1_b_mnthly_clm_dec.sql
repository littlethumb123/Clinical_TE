create or replace table `{{ dag_run.conf["DEC_DATASET"] }}.{{ dag_run.conf["prefix"] }}_monthly_claims`
...
select
	...
	and clm.srv_start_dt < DATE_SUB(base.index_dt, INTERVAL 90 DAY) 
        and IF(clm.paid_dt > DATE("1900-01-01"), clm.paid_dt, clm.adjn_dt) < DATE_SUB(base.index_dt, INTERVAL 90 DAY)  -- take from D_ for recent claims
