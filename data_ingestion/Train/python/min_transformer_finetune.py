...
try:
    blob = storage.Client(credentials=google.auth.default()[0]).bucket(bucket_name).blob(os.path.join(model_path,'bestModel_finetune'))
    bestModel = BytesIO()
    blob.download_to_file(bestModel)
    bestModel=joblib.load(bestModel)  
    best_model = bestModel['model']
...
try:
    blob = storage.Client(credentials=google.auth.default()[0]).bucket(bucket_name).blob(os.path.join(model_path,'checkpoint_finetune'))
    checkpoint = BytesIO()
    blob.download_to_file(checkpoint)
    checkpoint=joblib.load(checkpoint)  
    model = TransformerModel(nhead, nhid, nlayers, ndropout)
    model.decoder_cd = class_ip()
    ...
    target_cd_cnt = 3076 # numbr of target codes
    blob = storage.Client(credentials=google.auth.default()[0]).bucket(bucket_name).blob(os.path.join(model_path,'checkpoint'))
    checkpoint = BytesIO()
    blob.download_to_file(checkpoint)
    checkpoint=joblib.load(checkpoint)  
