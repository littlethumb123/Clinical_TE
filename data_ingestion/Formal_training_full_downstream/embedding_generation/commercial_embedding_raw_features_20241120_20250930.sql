/*==============================================================================
  COMMERCIAL EMBEDDING RAW FEATURES (CORRECTED)

  Why this script exists:
  - The source training pipeline in data_ingestion/TE_pretraining_data_ingestion/
    commercial_for_training.sql currently builds cohort membership with:
      eff_dt BETWEEN DATE('2023-01-01') AND DATE('2023-12-31')
  - Therefore, simply filtering a834793_Commercial_member_o3_train_ending for
  ...
  - Rebuild raw embedding features from source membership/claims/pharmacy data
    using the same transformation logic used by the Commercial pipeline for
  ...
  - 2024-11-20 to 2025-09-30 (inclusive)

  Output table:
  - edp-prod-storage.edp_ent_sdoheir_cns.a964286_commercial_embedding_raw_features_20241120_20250930
==============================================================================*/

DECLARE start_index_dt DATE DEFAULT DATE('2024-11-20');
DECLARE end_index_dt DATE DEFAULT DATE('2025-09-30');

-- Step 0a: one random index_dt per Commercial member in requested window
CREATE TEMP TABLE current_individual_id AS
SELECT
  member_id,
...
-- Step 0b: Commercial SI exclusions (same as source Commercial pipeline)
...
restricted_si AS (
  ...
  LEFT JOIN `edp-prod-hcbstorage.edp_hcb_core_cnsv.GROUP_CONTROL` g
    ON a.group_nbr = g.group_nbr
  LEFT JOIN `anbc-hcb-prod.insights_share_hcb_prod.v_enriched_customer` cum
    ON g.ps_unique_id = SAFE_CAST(cum.psuid AS INT64)
  WHERE a.fund_ctg_cd = 'B'
    AND cum.rstrctd_client_ind = 'Y'
),
termed_si AS (
  SELECT DISTINCT g.ps_unique_id
  FROM base_with_membership a
  LEFT JOIN `edp-prod-hcbstorage.edp_hcb_core_cnsv.GROUP_CONTROL` g
    ON a.group_nbr = g.group_nbr
  WHERE a.fund_ctg_cd = 'B'
    AND g.renewal_dt < DATE_SUB(CURRENT_DATE(), INTERVAL 24 MONTH)
)
SELECT DISTINCT
  base.member_id,
  base.individual_id,
...
AS 'No Commercial members found in requested index_dt window.';
...
DROP TABLE IF EXISTS `edp-prod-storage.edp_ent_sdoheir_cns.a964286_commercial_embedding_raw_features_20241120_20250930`;

CREATE TABLE `edp-prod-storage.edp_ent_sdoheir_cns.a964286_commercial_embedding_raw_features_20241120_20250930`
CLUSTER BY individual_id, index_dt
OPTIONS (
  labels=[("owner", "pritha_ghosh_cvshealth_com"), ("costcenter", "13070")],
  description="Commercial raw TE embedding features regenerated for index_dt 2024-11-20 to 2025-09-30",
