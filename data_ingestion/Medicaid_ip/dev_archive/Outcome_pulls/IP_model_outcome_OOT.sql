CREATE OR REPLACE TABLE `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_OOT_existing_scores`
...
AS
SELECT b.asdb_member_key, a.ip_risk, a.cdpsrx_scr, a.ip_rnk, a.cdpsrx_rnk, a.run_dt, b.final_score, b.final_riskstrat
FROM `anbc-hcb-prod.cm_medicaid_hcb_prod.medicaid_core2_0_final_score_history` AS a
RIGHT JOIN --`anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_member_index` AS b
  `anbc-hcb-dev.cm_medicaid_hcb_dev.a091749_all_cohort` AS b
    ON a.asdb_member_key = b.asdb_member_key
    AND a.run_dt = b.run_dt
;

CREATE OR REPLACE TABLE `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_OOT_outcome_ip_cases`
OPTIONS (labels = [("owner", "yourname_aetna_com"),("cost_center", "13070")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 180 DAY))
AS
SELECT 
...
CREATE OR REPLACE TABLE `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_OOT_outcome_ip`
OPTIONS (labels = [("owner", "yourname_aetna_com"),("cost_center", "13070")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 180 DAY))
AS
WITH acute AS (
    ...
    FROM  
        `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_OOT_outcome_ip_cases`
    WHERE 
        ip_type = "Acute"
    GROUP BY 
        asdb_member_key
