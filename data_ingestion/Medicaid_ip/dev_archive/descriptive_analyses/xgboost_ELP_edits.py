...
# # Assuming categorical_features is a list of categorical columns excluding 'index_dt' and 'gender'
# categorical_features = [col for col in df.columns if col not in ['index_dt', 'gender']]

# # Convert categorical columns to strings
# df[categorical_features] = df[categorical_features].astype(str)

# # Map gender values and fill NaNs
# # df['gender'] = df['gender'].map({'M': 1, 'F': 0}).fillna(-1)

# # Remove 'gender' from categorical features
...
#     counts = df[feature].value_counts().compute()
...
#         encoded_data = encoder.fit_transform(filtered_df[[feature]].compute())
#         encoded_df = pd.DataFrame(encoded_data, columns=encoder.get_feature_names_out([feature]))
       
#         # Convert the encoded DataFrame back to Dask DataFrame
#         encoded_df = dd.from_pandas(encoded_df, npartitions=4)
       
#         # Drop the original column and concatenate the new encoded columns
#         df = df.drop(feature, axis=1)
#         df = dd.concat([df, encoded_df], axis=1)

# # Convert the Dask DataFrame back to a Pandas DataFrame if needed
# df = df.compute()
...
# %%
# # Set GCP project/tenant constants
# PROJECT_ID = "anbc-hcb-dev"
# DATASET_ID = "cm_medicaid_hcb_dev"
# OWNER_EMAIL = "eric_ma_aetna_com"
# COST_CENTER = "13070"
# LOCATION = "US"
# MODEL_ID = "a996283_IP_2024"

# PROJECT_DATASET = F"{PROJECT_ID}.{DATASET_ID}"

# # Update source table.
# SOURCE_TABLE = f"{PROJECT_DATASET}.{MODEL_ID}_features_onehot"

# # Update target table which results will be written.
# TARGET_TABLE = f"{PROJECT_DATASET}.{MODEL_ID}_features_onehot"


# ################ TEST ################
# PROJECT_ID, DATASET_ID, OWNER_EMAIL, COST_CENTER, LOCATION, PROJECT_DATASET, SOURCE_TABLE, TARGET_TABLE

# %%
# client = bigquery.Client(project=PROJECT_ID, location=LOCATION)
# print("BQ client created using default project: {}".format(client.project))
...
# query_config = bigquery.QueryJobConfig(labels=bq_labels_dict)
# print("BQ query_config created...")
      
# load_config = bigquery.LoadJobConfig(labels=bq_labels_dict)
# print("BQ load_config created...")

# %%
# def write_table(data, TARGET_TABLE, client, query_config, load_config):
    
#     # Drop temp table
#     drop_temp_table_query=f"""DROP TABLE IF EXISTS {TARGET_TABLE}_tmp"""
#     drop_tmp_job = client.query(drop_temp_table_query, job_config=query_config)
#     drop_tmp_job.result()
    
#     # Write results into temp table
#     load_job = client.load_table_from_dataframe(
#     dataframe=data,
#     destination=f"{TARGET_TABLE}_tmp",
...
#     drop_job = client.query(
#         f"DROP TABLE IF EXISTS {TARGET_TABLE};", job_config=query_config
#     )

#     drop_job.result()
#     print("Final table is dropped")
    
#     alter_job = client.query(
#     f"ALTER TABLE `{TARGET_TABLE}_tmp` RENAME TO {MODEL_ID}_final_train_data_cp",
#     job_config=query_config,
#     )

#     alter_job.result()
#     print(f"Table is saved to {TARGET_TABLE}")
    
# write_table(df, TARGET_TABLE, client, query_config, load_config)
...
# SELECT *
# FROM 
#     `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_embeddings` AS o
# """ #WHERE 1=1 AND o.asdb_member_key IN (SELECT asdb_member_key FROM `anbc-hcb-dev.cm_medicaid_hcb_dev.a534354_IP_2024_non_embedding_features` WHERE 1=1 AND NOT asdb_plan_key IN (33, 54) AND post_mnths >= 6)
...
# df = df_og
# df['index_dt']
# df = pd.get_dummies(df, columns=['primarylanguage_desc'], prefix='lang', dummy_na=True)
# df_og['primarylanguage_desc']
# df.drop(columns=[col for col in df if col.startswith('lang_') and col.split('_')[-1] not in filtered_langs], inplace=True)
...
# %%
undersample = RandomUnderSampler(sampling_strategy=0.03, random_state=53)  
steps = [
    ('u', undersample)
]
...
#     n_jobs=32,
#     verbosity=0,
#     use_label_encoder=False,  # To avoid a warning about label encoding
#     enable_categorical=True,  # Enable handling of categorical data directly
#     tree_method='gpu_hist',
#     predictor = 'gpu_predictor'
# )

# # TODO: 
# # Dump model into memory
# # Empty memory each iteration for not need
# # RFE Garbage parameter

# rfecv = RFECV(
#     estimator=xgb_model_rfe,
#     step=10,  # Number of features to remove at each step
#     cv=StratifiedKFold(3),  # Number of folds for cross-validation
#     scoring="roc_auc",
#     min_features_to_select=1,
#     n_jobs=32  # Use all processors
# )
...
# # catboost_model_rfe.fit(X_train, y_train, eval_set=Pool(X_val, y_val), verbose=0, plot=False, use_best_model=True)#, cat_features=string_columns

# X_train = rfecv.transform(X_train) 
# explainer = shap.TreeExplainer(catboost_model_rfe)
...
# # Assuming 'rfecv' is your RFECV object and 'X_train' is a NumPy array

# # # Create feature names based on indices if none exist
# # feature_names = [f"Feature_{i}" for i in range(559)]
# # # print(feature_names)

# # Get the boolean mask of selected features and feature rankings
...
# assert len(feature_names) == len(selected_features_mask), "Feature names and selected mask are not the same length"
# assert len(feature_names) == len(feature_rankings), "Feature names and rankings are not the same length"

# # DataFrame with feature indices as names, their corresponding selection status, and rankings
...
# features_df.to_csv('selected_features_and_rankings.csv', index=False)
...
# plt.xlabel("Number of features selected")
# plt.ylabel("Mean test accuracy")
# plt.plot(cv_results["n_features"], cv_results["mean_test_score"], marker='o')
# plt.title("Recursive Feature Elimination with correlated features")
# plt.grid(True)
# plt.savefig('RFE_CV_Plot.png')
# plt.show()

# print("Files saved: 'selected_features_and_rankings.csv' and 'RFE_CV_Plot.png'")




# %%
# # Update selected_features_and_rankings.csv with feature names
# df = pd.read_csv('selected_features_and_rankings.csv')
# df['Feature'] = df.
...
def calculate_metrics(model):
    ...
    top_10_percent = int(0.1 * len(y_test_reset))  # 10% of test data size
    predicted_top_10_percent = y_test_reset.iloc[idx][:top_10_percent]
    actual_positives_top_10_percent = predicted_top_10_percent.sum()
    expected_positives = y_test_reset.sum() * 0.1
    lift_10_perc = actual_positives_top_10_percent / expected_positives

    
    top_cutoff = int(0.01 * len(y_test_reset))  # Calculate cutoff index for top X%
    y_true_top = y_test_reset.iloc[idx][:top_cutoff]
    tn, fp, fn, tp = confusion_matrix(y_test_reset, [1]*len(y_true_top)+[0]*(len(y_test_reset)-len(y_true_top)), labels=[0,1]).ravel()
    ppv_1_perc = 100*(tp / (tp + fp)) if (tp + fp) > 0 else 0
    sensitivity_1_perc = 100*(tp / (tp + fn)) if (tp + fn) > 0 else 0
    
    top_cutoff = int(0.1 * len(y_test_reset))  # Calculate cutoff index for top X%
    y_true_top = y_test_reset.iloc[idx][:top_cutoff]
    tn, fp, fn, tp = confusion_matrix(y_test_reset, [1]*len(y_true_top)+[0]*(len(y_test_reset)-len(y_true_top)), labels=[0,1]).ravel()
    ppv_10_perc = 100*(tp / (tp + fp)) if (tp + fp) > 0 else 0
    sensitivity_10_perc = 100*(tp / (tp + fn)) if (tp + fn) > 0 else 0
