...
TEMPORARY_GCS_BUCKET = 'cm-medicaid-code-hcb-dev'
...
SOURCE_TABLE = PROJECT_ID + '.' + DATASET_ID + '.medicaid_ip_features'
TARGET_TABLE = 'medicaid_IP_predict'
...
#model.load_model("cacm-mdcd-new_member_model/descriptive_analyses/xgboost_results/No_BH/xgboost_model_elle_check.cbm")
...
def create_prediction_table(index, y_pred):
    print("STEP 3: Creating the prediction table...")
...
def combiner(SOURCE_TABLE, TARGET_TABLE, model):
    start_time = time.time()
    data = load_data_from_bq(SOURCE_TABLE)
    data.show(5)
    feature_pd = data.toPandas()
    y_pred = make_prediction(model, feature_pd)
...
combiner(SOURCE_TABLE, TARGET_TABLE, model)    
