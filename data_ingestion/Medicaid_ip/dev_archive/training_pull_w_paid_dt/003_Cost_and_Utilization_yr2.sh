...
'DROP TABLE IF EXISTS `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_op_yr2_v2`'

bq query \
--use_legacy_sql=false \
'
CREATE TABLE  `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_op_yr2_v2`
    ...
    SELECT 
        *
        ...
        , CASE WHEN ROW_NUMBER() OVER(PARTITION BY asdb_member_key, asdb_plan_key, asdb_svc_prov_key, asdb_incurred_dt) = 1 THEN 1 
            ...
            END AS op_ct
    FROM 
        `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_med_claims_yr2_v2`
    WHERE 
...
bq query \
--use_legacy_sql=false \
'DROP TABLE IF EXISTS `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_med_claims_flag_yr2_v2`'

bq query \
--use_legacy_sql=false \
'
CREATE TABLE `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_med_claims_flag_yr2_v2`
--PARTITION BY RANGE_BUCKET(asdb_plan_key, GENERATE_ARRAY(0,100,1))
OPTIONS (labels = [("owner", "'$OWNER'"),("cost_center", "'$COST_CENTER'")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), '$DEFAULT_EXP'))
AS
...
bq query \
--use_legacy_sql=false \
'DROP TABLE IF EXISTS `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_other_cost_utilization_yr2_v2`'

bq query \
--use_legacy_sql=false \
'
CREATE TABLE `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_other_cost_utilization_yr2_v2`
--PARTITION BY RANGE_BUCKET(asdb_plan_key, GENERATE_ARRAY(0,100,1))
OPTIONS (labels = [("owner", "'$OWNER'"),("cost_center", "'$COST_CENTER'")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), '$DEFAULT_EXP'))
AS
...
FROM 
    (SELECT DISTINCT asdb_member_key, asdb_plan_key, index_dt FROM `'$ST'`) AS st
LEFT JOIN 
    `'$GCP_PROJECT'.'$GCP_DB'.'$PREFIX'_med_claims_flag_yr2_v2` AS clm
        ON st.asdb_member_key=clm.asdb_member_key
        AND st.asdb_plan_key=clm.asdb_plan_key
GROUP BY 
    st.asdb_member_key
