...
file_name = "MDCD_IP_2024_xgb_v_1_3_1.pkl"

### NOTE: change below to the table used in production ###
TARGET_TABLE ="cm_medicaid_hcb_dev.a534354_medicaid_IP_predict_v_1_3_1"
...
df = df.rename(columns={'coa_population_group_TANF_CHIP': 'coa_population_group_TANF/CHIP', 
                        'coa_population_group_Dual_Elig': 'coa_population_group_Dual Elig', 
                        #'emb64':'emb94',
                       })
df.set_index('asdb_member_key', inplace = True)

df = df.astype({col: np.int64 for col in df.columns if df[col].dtype == 'Int64'})
df = df.astype({col: np.float64 for col in df.columns if df[col].dtype == 'float64'})
