...
drop table if exists `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_current`;
create table `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_current`
...
drop table if exists `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_y2020`;
create table `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_y2020`
    OPTIONS ( labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")])
as
with birth_dt as (
    select a.individual_id,
...
drop table if exists `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_y2018`;
create table `{{DEC_TARGET_DB}}.{{PREFIX}}_rx_y2018`
    OPTIONS ( labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")])
as
with birth_dt as (
    select a.individual_id,
...
drop table if exists `{{DEC_TARGET_DB}}.{{PREFIX}}_d1c_score_ending_tmp`;
create table `{{DEC_TARGET_DB}}.{{PREFIX}}_d1c_score_ending_tmp`
    OPTIONS ( labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")])
as
