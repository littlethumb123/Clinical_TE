...
-- 
-- create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_archive_edw_clms_y2021`
...
create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_archive_edw_clms_y2020`
OPTIONS (labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")]) as
with birth_dt as (
    select a.individual_id,
           a.index_dt,
...
-- 
-- create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_archive_edw_clms_y2019`
-- OPTIONS (labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")]) as
-- with birth_dt as (
--     select a.individual_id,
--            a.member_id,
...
create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_archive_edw_clms_y2018`
OPTIONS (labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")]) as
with birth_dt as (
    select a.individual_id,
           a.index_dt,
...
create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_archive_edw_clms`
OPTIONS (labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")]) as 
