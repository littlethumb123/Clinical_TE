-- 
-- create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_icd9_y2021`
...
create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_icd9_y2020`
    OPTIONS ( labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")])
as
 select
         base.individual_id,
     ...
     from
         ( select individual_id,index_dt,member_id,claim_line_id, srv_start_dt
           from `{{DEC_TARGET_DB}}.{{PREFIX}}_d1a_score_ending_tmp`
...
-- 
-- create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_icd9_y2019`
...
create or replace table `{{DEC_TARGET_DB}}.{{PREFIX}}_icd9_y2018`
    OPTIONS ( labels=[("owner", "{{OWNER}}"),("costcenter", "{{COSTCENTER}}")]) as
     select
        base.individual_id,
        base.index_dt,
